...
UPDATE SINH_VIEN SET DuocKhenThuong = 1 WHERE MaSV IN ( SELECT NTV.MaSV FROM NHOM_THANHVIEN NTV JOIN NHOM N ON NTV.MaNhom = N.MaNhom JOIN PHAN_CONG_CONG_VIEC PCCV ON PCCV.MaNhom = N.MaNhom JOIN GIAO_VIEN GV ON PCCV.MaGV = GV.MaGV JOIN CONG_VIEC CV ON PCCV.MaCV = CV.MaCV JOIN AP ON CV.MaAp = AP.MaAp JOIN SINH_VIEN SV2 ON SV2.MaSV = NTV.MaSV WHERE GV.MaKhoa = 'K001' AND AP.TenAp LIKE N'Ấp A%' AND NTV.MaSV <> N.TruongNhom AND SV2.MaKhoa = 'K004' );
